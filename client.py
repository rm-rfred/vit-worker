...
def _initialize_worker(server_address: str) -> None:
    ...
    _worker_stub_singleton = image_classification_pb2_grpc.ImageClassificationServiceStub(_worker_channel_singleton)
...
def _run_worker_query(img: bytes) -> str:
    ...
    """
    response: image_classification_pb2.ImageClassificationResult = _worker_stub_singleton.Detect(
        image_classification_pb2.ImageClassificationCandidate(image=img)
    )
...
def compute_detections(batch: tp.List[bytes]) -> tp.List[str]:
    ...
    with multiprocessing.Pool(
        ...
        classification_results = worker_pool.map(
            _run_worker_query, [pickle.dumps(img) for img in batch]
        )
        return [txt for txt in classification_results]
...
def run():
    logger.info("IMAGECLASSIFICATION Test client started.")
    batch = prepare_batch()
